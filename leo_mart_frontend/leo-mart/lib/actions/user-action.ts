...
export async function fetchBusinessAccountsAction(params: {
  ...
  try {
    ...
    const url = `${BACKEND_URL}/api/v1/admin/users/business-accounts?page=${page}&size=${size}${status ? `&status=${status}` : ""}`;
    ...
    console.error("fetchBusinessAccountsAction error:", error);
    return { success: false, message: error.message || "Failed to fetch business accounts" };
  }
}

export async function updateBusinessAccountStatusAction(
  userId: string,
  ...
  try {
    ...
    const res = await fetch(`${BACKEND_URL}/api/v1/admin/users/${userId}`, {
      ...
      body: JSON.stringify({ businessAccount: { status } }),
    });

    const result = await res.json();
    if (result.success) {
      revalidatePath("/admin/dashboard");
      return { success: true, message: `Business account ${status}` };
    }
    return { success: false, message: result.message || "Failed to update business account" };
  } catch (error: any) {
    return { success: false, message: error.message || "Failed to update business account" };
  }
}
